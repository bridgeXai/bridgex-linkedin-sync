...
import { TABLE_B_FIELDS, formatCounterForBitable } from "./table-b-constants.js";

const FEISHU_ERROR_HINTS = {
  TextFieldConvFail:
    "飞书字段类型不匹配。表 C：sent_at 列类型或 personalized_excerpt 过长。表 B：connect_sent 须为数字列，connect_accepted/messages_sent/replies_received 须为文本列（见 SESSION-HANDOFF §三.1）。",
...
function formatCounterValue(fieldName, nextValue) {
  return formatCounterForBitable(fieldName, nextValue);
}
...
async function incrementDailyLogCounter({ token, appToken, tableId, accountId, actionType }) {
  ...
  if (counterField !== TABLE_B_FIELDS.connectSent) {
    fields[TABLE_B_FIELDS.connectSent] = formatCounterForBitable(TABLE_B_FIELDS.connectSent, 0);
  }
  if (counterField !== TABLE_B_FIELDS.messagesSent) {
    fields[TABLE_B_FIELDS.messagesSent] = formatCounterForBitable(TABLE_B_FIELDS.messagesSent, 0);
  }
  if (counterField !== TABLE_B_FIELDS.repliesReceived) {
    fields[TABLE_B_FIELDS.repliesReceived] = formatCounterForBitable(TABLE_B_FIELDS.repliesReceived, 0);
  }
  if (counterField !== TABLE_B_FIELDS.connectAccepted) {
    fields[TABLE_B_FIELDS.connectAccepted] = formatCounterForBitable(TABLE_B_FIELDS.connectAccepted, 0);
  }
